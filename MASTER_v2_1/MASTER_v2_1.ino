 ...
 * 
 * Based on: master_demo_v1_1.ino (working version)
 * Added:
 * - Full gun/light/mast commands support
 * - Device auto-cleanup (offline detection)
 * - Extended JSON with device parameters
 * - Support for 30+ slaves
 */
...
String getDevicesJSON() {
  ...
  for (auto &p : devices) {
    // Skip offline devices (removed from list)
...
void setup() {
  ...
  server.on("/cmd", HTTP_POST, [](AsyncWebServerRequest *r) {
    ...
    // Add parameters to command if present
    if (r->hasParam("value", true)) {
      cmd += "," + r->getParam("value", true)->value();
    }
    
    // For LIGHT_COLOR command
